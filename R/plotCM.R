...
plotCM<-function(tbl=NULL,
  ...
  if (is.null(tbl)){
    tbl<-getCSV(csvfile,caption="Select DisMELS connectivity matrix file");
  }
  
  if (is.data.frame(tbl)) plotCM.dataframe(tbl,zmax=zmax,zmin=zmin,
                                           title=title,xlab=xlab,ylab=ylab,
                                           devtype=devtype,cex=cex,colorscale=colorscale,
                                           plotfile=plotfile,plotsize=plotsize);
  if (is.matrix(tbl))     plotCM.matrix(tbl,zmax=zmax,zmin=zmin,
                                        title=title,xlab=xlab,ylab=ylab,
                                        devtype=devtype,cex=cex,colorscale=colorscale,
                                        plotfile=plotfile,plotsize=plotsize);
}
